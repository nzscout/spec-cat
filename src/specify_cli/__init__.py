...
def _enable_windows_console_fallback() -> None:
    ...
    Upstream reconfigures stdout/stderr to UTF-8 inside ``main()``, but that
    only covers the ``specify`` entry point. The CL ``speckit`` command is a
    separate entry point, so this import-time guard keeps both paths safe by
    switching non-UTF-8 streams to ``errors="replace"``.
    """
    if os.name != "nt":
        return
