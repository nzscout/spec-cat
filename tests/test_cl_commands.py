...
from specify_cli.cl_commands import (
    _claude_extra_skill_name,
    _copy_extras,
    _iter_cl_prompt_extras,
    _render_claude_extra_skill,
    _render_codex_extra_skill,
)
...
class TestCodexClExtras:
    def test_render_codex_skill_from_agent_backed_prompt(self):
        prompt_path = next(
            path for path in _iter_cl_prompt_extras() if path.name == "speckat.compare-code.prompt.md"
        )

        content = _render_codex_extra_skill(prompt_path)
        frontmatter, _ = CommandRegistrar.parse_frontmatter(content)

        assert frontmatter["name"] == "speckat-compare-code"
        assert frontmatter["compatibility"] == "Requires spec-kit project structure with .specify/ directory"
        assert frontmatter["metadata"]["source"] == "cl-tools/extras/.github/prompts/speckat.compare-code.prompt.md"
        assert "# Persona and core behavior" in content
        assert "## User Input" in content

    def test_copy_extras_installs_codex_skills_when_codex_selected(self, tmp_path):
        specify_dir = tmp_path / ".specify"
        specify_dir.mkdir()
        (specify_dir / "init-options.json").write_text(
            json.dumps({"ai": "codex"}), encoding="utf-8"
        )
        ...
        assert copied > 0
        assert updated == 0
        assert (tmp_path / ".agents" / "skills" / "speckat-compare-code" / "SKILL.md").exists()
        assert not (tmp_path / ".claude" / "skills" / "speckat-compare-code" / "SKILL.md").exists()
